...
int main(int argc, char *argv[]){
	...
	start=clock();
	for(timesRepeated=0;timesRepeated<10000;timesRepeated++){
		rewind(readsFile);
		rewind(hitsFile);
...
void* processRead(void *args){
	...
	threadEnd=clock();
	thread_time+=((double)(threadEnd-threadStart))/CLOCKS_PER_SEC;
