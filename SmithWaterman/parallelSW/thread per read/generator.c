...
char* generateDNAFromReads(char** reads, int numOfReads, int coverage, int diffPerRead);
...
int main(int argc, char *argv[]){
	int dnaLength, numOfReads, readLength, coverage;
	char* dna;
	char** reads;
	int i;
	...
	dna=generateDNAFromReads(reads,numOfReads,coverage,0);
...
char* generateDNAFromReads(char** reads, int numOfReads, int coverage, int diffPerRead){
	int i,index,k;
	char *read,*dna;
	char **readsForDNA=(char**)malloc((2*numOfReads*coverage+1)*sizeof(char*));
	if(readsForDNA==NULL){
		printf("Error: cannot allocate memory\n");
		return NULL;	
	}
	
	for(i=0;i<numOfReads*coverage;i++){
		index=rand()%numOfReads;	
		read=strdup(reads[index]);
		
		addDeletion(read,4,1,5);
		addMismatch(read,4,1,4);
		read=addInsertion(read,4,1,5);
		
		readsForDNA[i]=read;
		
		i++;
		k=rand()%20;
		readsForDNA[i]=generateDNA(k);
	}
	readsForDNA[i]=NULL;
