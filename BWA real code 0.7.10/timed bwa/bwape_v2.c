...
float time_for_bwa_cal_pac_pos_pe=0,time_for_sw=0,time_for_cal_pac_pos_pe_second_block=0,first_block=0,second_block=0,third_block=0,second_block_first=0,second_block_second=0,second_block_third=0;
...
fprintf(stderr, "time in infer_isize: %.2f sec\n", (float)(clock() - tic) / CLOCKS_PER_SEC);
...
tic=clock();
toc=clock();
fprintf(stderr,"bwa_cal_pac_pos_pe\n");
	int i, j, cnt_chg = 0;
	char str[1024];
	bwt_t *bwt;
	pe_data_t *d;
...
fprintf(stderr, "PAC POS 1st block: %.2f sec\n", (float)(clock() - toc) / CLOCKS_PER_SEC);toc=clock();
	// PE
	for (i = 0; i != n_seqs; ++i) {
		bwa_seq_t *p[2];
timer=clock();
...
//fprintf(stderr, "cal_pac_pos_pe pe part1: %.2f sec\n", (float)(clock() - timer) / CLOCKS_PER_SEC);
first_block+=(float)(clock() - timer) / CLOCKS_PER_SEC;
timer=clock();
...
second_block_first+=(float)(clock() - tac) / CLOCKS_PER_SEC;
tac=clock();
...
second_block_second+=(float)(clock() - tac) / CLOCKS_PER_SEC;
tac=clock();
			cnt_chg += pairing(p, d, opt, gopt->s_mm, ii);
second_block_third+=(float)(clock() - tac) / CLOCKS_PER_SEC;
tac=clock();
		}
//fprintf(stderr, "cal_pac_pos_pe pe part2: %.2f sec\n", (float)(clock() - timer) / CLOCKS_PER_SEC);
second_block+=(float)(clock() - timer) / CLOCKS_PER_SEC;
timer=clock();
...
//fprintf(stderr, "cal_pac_pos_pe pe part3: %.2f sec\n", (float)(clock() - timer) / CLOCKS_PER_SEC);
third_block+=(float)(clock() - timer) / CLOCKS_PER_SEC;
timer=clock();
	}
time_for_cal_pac_pos_pe_second_block+=(float)(clock() - toc) / CLOCKS_PER_SEC;
fprintf(stderr, "PAC POS 2nd block: %.2f sec\n", (float)(clock() - toc) / CLOCKS_PER_SEC);toc=clock();
	// free
	for (i = 0; i < n_seqs; ++i) {
	...
	free(d);
fprintf(stderr, "PAC POS 3rd block: %.2f sec\n", (float)(clock() - toc) / CLOCKS_PER_SEC);toc=clock();	
fprintf(stderr, "time in bwa_cal_pac_pos_pe: %.2f sec\n", (float)(clock() - tic) / CLOCKS_PER_SEC);
time_for_bwa_cal_pac_pos_pe+=(float)(clock() - tic) / CLOCKS_PER_SEC;
	return cnt_chg;
}
...
//fprintf(stderr,"bwa_sw_core-before ksw_align\n");
align_counter++;
...
//fprintf(stderr,"bwa_sw_core-after ksw_align\n");
...
//fprintf(stderr,"number of calls to align: %d\n",align_counter);
...
{
fprintf(stderr,"bwa_sai2sam_pe_core\n");
...
toc=clock();
		...
		last_ii = ii;
time_for_sw+=(float)(clock() - toc) / CLOCKS_PER_SEC;
	}
fprintf(stderr,"NUM OF CALLS TO ALIGN: %d\n",align_counter);
fprintf(stderr, "TOTAL TIME FOR PAC POS PE 2nd BLOCK: %.2f sec\n",time_for_cal_pac_pos_pe_second_block);
fprintf(stderr, "TOTAL TIME FOR PAC POS PE: %.2f sec\n",time_for_bwa_cal_pac_pos_pe);
fprintf(stderr, "TOTAL cal_pac_pos_pe pe part1: %.2f sec\n", first_block);
fprintf(stderr, "TOTAL cal_pac_pos_pe pe part2: %.2f sec\n", second_block);
fprintf(stderr, "\tTOTAL cal_pac_pos_pe pe part2 first block: %.2f sec\n", second_block_first);
fprintf(stderr, "\tTOTAL cal_pac_pos_pe pe part2 second block: %.2f sec\n", second_block_second);
fprintf(stderr, "\tTOTAL cal_pac_pos_pe pe part2 third block: %.2f sec\n", second_block_third);
fprintf(stderr, "TOTAL cal_pac_pos_pe pe part3: %.2f sec\n", third_block);

fprintf(stderr, "TOTAL TIME FOR SW: %.2f sec\n",time_for_sw);
	// destroy
	bns_destroy(bns);
	for (i = 0; i < 2; ++i) {
		bwa_seq_close(ks[i]);
