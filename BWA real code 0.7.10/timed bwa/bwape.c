...
float 		sam_time=0,
		sam_initialize_time=0,
		sam_core_time=0,
		pos_pe_time=0,
		pos_pe_initialize_time=0,
		pos_pe_SE_time=0,
		pos_pe_infer_time=0,
		pos_pe_PE_time=0,
		pos_pe_free_time=0,
...
/*
clock_t tic,toc,timer,tac;
float pac_pos=0,pac_pos_inner=0,pac_pos_inner_first=0,pac_pos_inner_second=0;
*/
					   ...
					   const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii)
{
	...
	//************************************
	pos_pe_initialize_time+=(float)(clock() - pos_pe_inner_timer) / CLOCKS_PER_SEC;
	pos_pe_inner_timer=clock();
	//************************************
	...
	//************************************
	pos_pe_SE_time+=(float)(clock() - pos_pe_inner_timer) / CLOCKS_PER_SEC;
	pos_pe_inner_timer=clock();
	//************************************
	...
	//************************************
	pos_pe_infer_time+=(float)(clock() - pos_pe_inner_timer) / CLOCKS_PER_SEC;
	pos_pe_inner_timer=clock();
	//************************************
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		//************************************
		PE_for_time+=(float)(clock() - PE_inner_timer) / CLOCKS_PER_SEC;
		PE_inner_timer=clock();
		//************************************
		...
		{ // only when both ends mapped
			...
			//************************************
			if_first_for_time+=(float)(clock() - PE_if_timer) / CLOCKS_PER_SEC;
			//************************************
			...
			for (j = 0; j < 2; ++j) {
				for (k = 0; k < d->aln[j].n; ++k) {
					...
					//************************************
					in_hash_time+=(float)(clock() - hash_timer) / CLOCKS_PER_SEC;
					hash_timer=clock();
					//************************************
						...
						for (l = r->k; l <= r->l; ++l) {
							...
							//************************************
							sa2pos_time+=(float)(clock() - timer) / CLOCKS_PER_SEC;
							timer=clock();
							//************************************
							x.y = k<<2 | strand<<1 | j;
							kv_push(pair64_t, d->arr, x);
							...
							//************************************
							kv_push_time+=(float)(clock() - timer) / CLOCKS_PER_SEC;
							//************************************
						}
					}
					//************************************
					not_in_hash_time+=(float)(clock() - hash_timer) / CLOCKS_PER_SEC;
					//************************************
				}
			}
			
			//************************************
			if_second_for_time+=(float)(clock() - PE_if_timer) / CLOCKS_PER_SEC;
			PE_if_timer=clock();
			//************************************
			...
			//************************************
			if_pairing_time+=(float)(clock() - PE_if_timer) / CLOCKS_PER_SEC;
			//************************************
		}

		//************************************
		PE_first_if_time+=(float)(clock() - PE_inner_timer) / CLOCKS_PER_SEC;
		PE_inner_timer=clock();
		//************************************
		...
		//************************************
		PE_second_if_time+=(float)(clock() - PE_inner_timer) / CLOCKS_PER_SEC;
		PE_inner_timer=clock();
		//************************************
	...
	//************************************
	pos_pe_PE_time+=(float)(clock() - pos_pe_inner_timer) / CLOCKS_PER_SEC;
	pos_pe_inner_timer=clock();
	//************************************
	...
	//************************************
	pos_pe_free_time+=(float)(clock() - pos_pe_inner_timer) / CLOCKS_PER_SEC;
	pos_pe_time+=(float)(clock() - pos_pe_timer) / CLOCKS_PER_SEC;	
	//************************************	
...
void bwa_sai2sam_pe_core(const char *prefix, char *const fn_sa[2], char *const fn_fa[2], pe_opt_t *popt, const char *rg_line)
{
	...
	//************************************
	sam_initialize_time=(float)(clock() - sam_inner_timer) / CLOCKS_PER_SEC;
	sam_inner_timer=clock();
	//************************************
	...
	//************************************
	sam_core_time=(float)(clock() - sam_inner_timer) / CLOCKS_PER_SEC;
	sam_time=(float)(clock() - sam_timer) / CLOCKS_PER_SEC;
	
	fprintf(stderr, "\n");
	fprintf(stderr, "SAM_PE_CORE TOTAL TIME: %.2f sec\n",sam_time);
	
	fprintf(stderr, "\tINITIALIZE TIME: %.2f sec\n",sam_initialize_time);
	fprintf(stderr, "\tCORE TIME: %.2f sec\n",sam_core_time);
	
	
	fprintf(stderr, "\t\tCAL PAC POS PE TIME: %.2f sec\n",pos_pe_time);	
	
	fprintf(stderr, "\t\t\tINITIALIZE TIME: %.2f sec\n",pos_pe_initialize_time);
	fprintf(stderr, "\t\t\tSE TIME: %.2f sec\n",pos_pe_SE_time);
	fprintf(stderr, "\t\t\tINFER TIME: %.2f sec\n",pos_pe_infer_time);
	fprintf(stderr, "\t\t\tPE TIME: %.2f sec\n",pos_pe_PE_time);
	fprintf(stderr, "\t\t\t\tFOR TIME: %.2f sec\n",PE_for_time);
	fprintf(stderr, "\t\t\t\tFIRST IF TIME: %.2f sec\n",PE_first_if_time);
	fprintf(stderr, "\t\t\t\t\tFIRST FOR TIME: %.2f sec\n",if_first_for_time);
	fprintf(stderr, "\t\t\t\t\tSECOND FOR TIME: %.2f sec\n",if_second_for_time);
	fprintf(stderr, "\t\t\t\t\t\tIF TIME: %.2f sec\n",in_hash_time);
	fprintf(stderr, "\t\t\t\t\t\tELSE TIME: %.2f sec\n",not_in_hash_time);
	fprintf(stderr, "\t\t\t\t\t\t\tBWA_SA2POS TIME: %.2f sec\n",sa2pos_time);
		fprintf(stderr, "\t\t\t\t\t\t\t\tBWASE.C - BWA_SA2POS\n");
		fprintf(stderr, "\t\t\t\t\t\t\t\tBWT_SA TIME: %.2f sec\n",bwt_sa_time);
			fprintf(stderr, "\t\t\t\t\t\t\t\t\tBWT.C - BWT_SA\n");
			fprintf(stderr, "\t\t\t\t\t\t\t\t\tBWT_INVPSI TIME: %.2f sec\n",invPsi_time);	
			fprintf(stderr, "\t\t\t\t\t\t\t\t\t\tBWT_B0 TIME: %.2f sec\n",bwt_B0_time);
			fprintf(stderr, "\t\t\t\t\t\t\t\t\t\tBWT_OCC TIME: %.2f sec\n",bwt_occ_time);
	
			
		fprintf(stderr, "\t\t\t\t\t\t\t\tBNS_DEPOS TIME: %.2f sec\n",bns_depos_time);
	
	fprintf(stderr, "\t\t\t\t\t\t\tKV_PUSH TIME: %.2f sec\n",kv_push_time);
	
	fprintf(stderr, "\t\t\t\t\tPAIRING TIME: %.2f sec\n",if_pairing_time);

	fprintf(stderr, "\t\t\t\tSECOND IF TIME: %.2f sec\n",PE_second_if_time);
	
	
	fprintf(stderr, "\t\t\tFREE TIME: %.2f sec\n",pos_pe_free_time);
	//************************************	
